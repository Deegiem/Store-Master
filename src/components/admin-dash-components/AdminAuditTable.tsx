...
import { formatActionText } from "@/utils/formatText"
...
export function AdminAuditTable({ logs }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Table Container - This is where the scrollbar should be */}
      <div className="overflow-x-auto">
        <div className="min-w-[800px]">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-4 py-3 sm:px-5 sm:py-4 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-600">
                  <div className="flex items-center gap-2">
            ...
            <tbody>
              {currentLogs.map((log, index) => (
                ...
                >
                  <td className="whitespace-nowrap px-4 py-3 sm:px-5 sm:py-4 text-xs text-slate-600">
                  ...
                  <td className="px-4 py-3 sm:px-5 sm:py-4 text-slate-600">
                    {log.target_type ? (
                      <div>
                  ...
                  <td className="px-4 py-3 sm:px-5 sm:py-4 text-xs text-slate-500 max-w-xs truncate">
                    {log.details || "-"}
                  </td>
                </motion.tr>
              ))}
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex flex-col gap-3 items-center justify-between border-t border-slate-200 px-4 py-3 sm:flex-row sm:px-5 sm:py-4">
              ...
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`h-7 w-7 sm:h-8 sm:w-8 rounded-sm text-xs sm:text-sm font-medium transition ${
                      currentPage === pageNum

...
import { Menu, X, LayoutDashboard, Package, Users, CircleUserRound, TableOfContents , LogOut, ChartNoAxesColumn, User, Container, ShoppingCart, BadgeDollarSign } from "lucide-react"
...
export default function Navigation() {
  ...
  const MENU_ITEMS = {
    ...
    "sales staff": [
      { name: "Dashboard", icon: LayoutDashboard, href: "/dashboard/staff" },
      { name: "Categories", icon: TableOfContents, href: "/dashboard/staff/categories" },
      { name: "Products", icon: Package, href: "/dashboard/staff/products" },
      { name: "Suppliers", icon: Container, href: "/dashboard/staff/suppliers" },
      { name: "Purchases", icon: ShoppingCart, href: "/dashboard/staff/purchases" },
      { name: "Sales", icon: BadgeDollarSign, href: "/dashboard/staff/sales" },
    ],
  }
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex flex-col space-y-1 rounded-b-md bg-[#ececee]">
          ...
          <div className="mt-8 p-4">
            <Link
                href={"/dashboard/admin/users/me"}
                className="w-full flex items-center justify-center px-4 space-x-2 py-2.5 bg-[#000ac0] text-white rounded-lg hover:bg-[#000343] transition"
            >
                <CircleUserRound />
                <p>Admin</p>
            </Link>
